...
public class Battle {
    ...
    //人物攻击力计算公式    double damage = attact * 1.5 * (Math.random()+0.5);
   ...
   //怪物攻击计算公式
    ...
    //怪物释放技能
     ...
     public int runAway(Player player,Monster monster){
        ...
        if(temp <0&& Math.random() < 0.7){ if(player.hpp<=0){definition = "逃跑成功";player.money-=50;if(player.money<10)player.money=10;new Textdialog(definition+"损失捡钱50两");return 1;}definition = "逃跑失败";
        new Textdialog(definition);
        return 0; 
       }
       if(temp >=0&& Math.random() >0.7){ if(player.hpp<=0){definition = "逃跑成功";player.money-=50;if(player.money<10)player.money=10;new Textdialog(definition+"损失捡钱50两");return 1;}definition = "逃跑失败";
        ...
        if(player.hpp<=0){player.money-=50;if(player.money<10)player.money=10;new Textdialog("损失捡钱50两");}
        definition = "逃跑成功";
        new Textdialog(definition);
        return 1;
     }
